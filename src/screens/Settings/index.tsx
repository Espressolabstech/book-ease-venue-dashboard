...
const Settings = () => {
        ...
        try {
            ...
            for (const { sport, offPeakPrice, peaks } of allPeaks) {
                ...
                for (const p of peaks) {
                    const dayStr = p.dayOfWeek !== null ? SHORT_DAYS[p.dayOfWeek] : '';
            ...
            setPeakConfigs(Object.values(bySport));
        } catch {
    ...
    const fetchDowntimes = useCallback(async () => {
        try {
            const res = (await listDowntimes()) as { data: { downtimes: ScheduledDowntime[] } };
    ...
    const toggleDay = (i: number) =>
        setHours((p) => p.map((h, idx) => (idx === i ? { ...h, isOpen: !h.isOpen } : h)));

    const updateTime = (i: number, field: 'openTime' | 'closeTime', v: string) =>
        setHours((p) => p.map((h, idx) => (idx === i ? { ...h, [field]: v } : h)));
    ...
    const togglePeakEnabled = (sport: string) => {
        setPeakConfigs((prev) =>
            prev.map((c) => c.sport === sport ? { ...c, peakEnabled: !c.peakEnabled } : c),
        );
    };

    const updateSportPrice = (sport: string, field: 'peakPrice' | 'offPeakPrice', value: number) => {
        setPeakConfigs((prev) =>
    ...
    const addPeakSlot = (sport: string) => {
        setPeakConfigs((prev) =>
            prev.map((c) =>
                c.sport === sport
                    ? { ...c, slots: [...c.slots, { id: `p${Date.now()}`, startTime: '', endTime: '', days: [] }] }
                    : c,
            ),
        );
    };

    const updatePeakSlot = (sport: string, slotId: string, updates: Partial<PeakHourSlot>) => {
        setPeakConfigs((prev) =>
            prev.map((c) =>
                c.sport === sport
                    ? { ...c, slots: c.slots.map((s) => s.id === slotId ? { ...s, ...updates } : s) }
                    : c,
            ),
        );
    };
    ...
    const savePeakHours = async () => {
        ...
        try {
            ...
            for (const config of peakConfigs) {
                ...
                await Promise.all(allExistingIds.map((id) => deletePeakHourPricing(id)));
            ...
            await Promise.all(
                peakConfigs.map((c) => {
                    ...
                    const sportCourts = courtsRaw.filter((r) => r.sport === apiSport);
                    return Promise.all(
                        sportCourts
    ...
    const saveEdit = async () => {
        ...
        try {
            ...
            await updateCourt(editForm.id, { name: editForm.name, environment, status });
            if (pricingId) {
                await updateCourtPricing(pricingId, { pricePerSlot: editForm.pricePerSlot });
            }
            cancelEdit();
    ...
    const commitCourt = async (court: Omit<CourtData, 'id'>) => {
        try {
            ...
            const timeSlots = hours
                .map((h, i) =>
                    h.isOpen ? { dayOfWeek: i, startTime: h.openTime, endTime: h.closeTime } : null,
                )
            ...
            await createCourt({
                ...
                timeSlots,
                pricing: { pricePerSlot: court.pricePerSlot },
            });
    ...
    const addDowntimeEntry = async () => {
        ...
        try {
            ...
            setNewDowntime({ courtId: '', startDate: '', endDate: '', startTime: '', endTime: '', reason: '' });
            ...
            <main className="mx-auto max-w-lg px-4 pt-4">
                ...
                {section === 'courts' && (
                    <CourtsSection
                        ...
                        onNewCourtChange={(u) => setNewCourt((p) => ({ ...p, ...u }))}
                        onEditFormChange={(u) => setEditForm((p) => (p ? { ...p, ...u } : p))}
                ...
                {section === 'policy' && (
                    <BookingRulesSection
                        ...
                        saving={savingPolicy}
                        onChange={(updates) => setPolicyDraft((p) => ({ ...p, ...updates }))}
                        onSave={savePolicy}
                    />
                )}
            </main>
                            ...
                            <div className="space-y-1.5">
                                ...
                                <div className="relative">
                                    ...
                                    <Input
                                        ...
                                        placeholder="150"
                                        className="h-11 text-base font-medium pl-7"
                                        autoFocus
                                    />
                                </div>
                            </div>
